...
const f32 Tile_Grid::tile_size = 16.f;
...
void Tile_Grid::debug_draw_grid(ch::Vector2 draw_pos, const ch::Color& color /*= ch::dark_gray*/) {
	...
	for (f32 x = draw_pos.x - half_width; x < draw_pos.x + half_width; x += tile_size) {
		const f32 rounded_x = round_to_grid(ch::Vector2(x, 0.f)).x + (tile_size / 2.f);
	...
	for (f32 y = draw_pos.y - half_width; y < draw_pos.y + half_width; y += tile_size) {
		const f32 rounded_y = round_to_grid(ch::Vector2(0.f, y)).y + (tile_size / 2.f);
